...
static uint32_t cto = 3000;

int air724_cmd_AT_RESET( void* airdev,int argc,char** argv )
{
	...
	{
		//读数据到dev->rxbuf
		rd_len = dev->uart->read_recv_buf( &(rxbuf[ dev->rxlen ]),10 );
		dev->rxlen += rd_len;
