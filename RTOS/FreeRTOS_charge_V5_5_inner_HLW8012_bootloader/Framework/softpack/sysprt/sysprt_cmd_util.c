...
static unsigned char process_one_line(char *cmd_line)
{
	...
	argc = split_string( cmd_line, argv," \t\n\r");
	if (argc == 0) return 0;
...
/*-----------------------------------------------------------------------*/
static int BootTime = 2;					//启动倒计时长度
static int en_count_down = 1;			//=1 使用开机倒计时,倒计时期间,可输入回车键,暂停系统启动
static int is_waiting = 1;				//描述是否正在等待启动
...
int console_work(void)
{
	...
	if( ( 1 == is_waiting )&& ( 1 == en_count_down ) )
	{
		...
		else
		{
			switch(buf[i])
			{
				...
				case 0X09:{
					...
					uint8_t TAB_item_buf[10];	/*用于存储TAB键的第一项*/
					int iLen = 0;
					PRT_LOG("\r\n ");
					...
					while( t_cmd_util != NULL)
					{
						if( strlib->strcmpn( (char*)line,t_cmd_util->name,Cursor) ){
							PRT_LOG("%s ",t_cmd_util->name);										
							TAB_item ++;
							/*如果存在候选命令，则复制第一条命令*/
							if(TAB_item==1) {
								for(i=0;i<10;i++) TAB_item_buf[i] = 0;
...
//	//倒计时程序
		//如果不是等待状态，则退出
		if( 0 == is_waiting )   break;
		
		//如果等待中，但是不倒计时了，则不执行下方倒计时读秒
		if( 0 == en_count_down ) continue;
