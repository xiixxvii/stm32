...
drv_uart_port_pt drvp_uart3 = &do_drvp_uart;
...
static void Init(uint32_t baudrate)
{
	...
	GPIO_Init( GPIOM, PIN6,0,0,1 );	//

	PORT_Init( PORTM, PIN6, FUNMUX0_UART3_RXD, 1 );	//
//	PORT_Init(PORTN, PIN5, FUNMUX1_UART3_TXD, 0);	//
	...
	/*设置奇偶校验------------------------------------------------------*/
	UART_initStruct.Parity = UART_PARITY_EVEN;//UART_PARITY_NONE;
	/*设置停止位----------------------------------------------------------*/
...
static void Open(void)			{UART_Open(UART3);}
static void Close(void)		{UART_Close(UART3);}
static void Write(unsigned char Dat)		
{ 
	while(1)
	{
		if( 0 == UART_IsTXBusy(UART3) )
		{
			break;
		}
	}	
	UART_WriteByte(UART3,Dat);
}
/*----------------------------------------------------------------------------------*/
...
//	if(UART_INTRXThresholdStat(UART3) || UART_INTTimeoutStat(UART3)){
//		while(UART_IsRXFIFOEmpty(UART3) == 0)	{//成立，说明RX FIFO非空
//			if(UART_ReadByte(UART3, &chr) == 0)	{thisDrvIntFunc(chr);}
//		}
//	}
#else	
	
	if(UART_INTStat(UART3, UART_IT_RX_THR | UART_IT_RX_TOUT))
	{
