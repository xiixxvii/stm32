...
uart_pt uart1 = &do_uart;
loopbuf_pt loopbuf_tx1;
loopbuf_pt loopbuf_rx1;
/*--------------------------------------------------------------------------------*/
static void init( uint32_t baudrate )
{
	//创建环形缓冲区
	loopbuf_tx1 = loop->create();
	loopbuf_rx1 = loop->create();
	
	//初始化串口
	drvp_uart = drvp_uart1;	//引用串口硬件
...
static void deal_data_send(void)
{
	//一次请求读10个数据
	uint8_t buf[10];
	int ret = loop->read( loopbuf_tx1,buf,10 );
...
static uint32_t read_recv_buf(uint8_t *buf,uint8_t len)
{
	return loop->read( loopbuf_rx1,buf,len ); 
	
}
static void clr_rbuf( void )
{
	loop->reset( loopbuf_rx1 );
}
