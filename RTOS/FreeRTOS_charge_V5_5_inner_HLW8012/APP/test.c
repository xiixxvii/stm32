...
uint8_t* str_src = "22020220708";
uint8_t* str_dst = "pL36EZh9+apsKMVkEObc+Q==";

static int test_encrypt_cbc(void)
{
	
	
  uint8_t key[] = "meikakjhuwenping";
    
	uint8_t iv[]  = "yaoqidixiaoye123";
	uint8_t* buffer = ( uint8_t* )mem_alloc( 1024 );
	memset( buffer , 0, 64);
	for( int i=0;i<strlen( (char*)str_src );i++ )
		buffer[i] = str_src[i];
	...
	for(i=0;i<64;i++) printf("%02x ",buffer[i]);	printf("\r\n");
	...
	printf("\r\n");
	for(i=0;i<16;i++) printf("%02x",buffer[i]);	printf("\r\n");
