...
//	air_man->send_str( dev,"\n" );
	...
	ret = strlib->dump( (char*)(&send_buf[ dump_len ]),"GET /skin2/save/fudai/riqi.php HTTP/1.1\r\n" );
	dump_len += ret;
	
	ret = strlib->dump( (char*)(&send_buf[ dump_len ]),"HOST: " );
	dump_len += ret;

	ret = strlib->dump( (char*)(&send_buf[ dump_len ]),serv_ip );
	dump_len += ret;

	ret = strlib->dump( (char*)(&send_buf[ dump_len ]),"\r\n" );
	dump_len += ret;

	ret = strlib->dump( (char*)(&send_buf[ dump_len ]),"Connection:Close\r\n\r\n" );
	dump_len += ret;
	
	PRT_LOG("------>\r\n%s\r\n",(char*)send_buf);	
//	for( int i=0;i<dump_len;i++ )
//	{
//		PRT_LOG("%02x ",send_buf[i]);	
//	}
	PRT_LOG("------\r\n");
	
	air_man->send_str(dev,(char*)send_buf);
